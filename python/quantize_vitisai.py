#!/usr/bin/env python3
"""Quantize a KataGo FP32 ONNX graph to INT8 QDQ for the WinML VitisAI (NPU) execution provider.

Invoked automatically by katago.exe's WinML backend when winmlProvider=vitisai and a bundled
Python environment + calibration data are found next to katago.exe (see Compiling.md). Requires
`amd-quark` (AMD's ONNX quantizer, providing the `quark.onnx` module), `onnx`, and `numpy`.

Can also be run manually:
    python quantize_vitisai.py --input model-fp32.onnx --calibration calib.npz --output model-int8.onnx

All progress/diagnostic output goes to stdout/stderr of THIS process; katago.exe redirects both to
a log file rather than inheriting them, so it's safe to print freely here.
"""
import argparse
import sys
...
class KataGoCalibrationDataReader:
    """Feeds calibration rows sampled by KataGo's dumpcalibrationdata/CalibrationData sampler.
...
def quantize(input_path, output_path, reader):
    ...
    # Fallback: the lower-level quantize_static function, with explicit flags approximating the
    # same scheme (NPU CNN target, QDQ format, INT8 activations/weights).
    reader.rewind()
