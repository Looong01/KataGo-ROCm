...
KataGo has six backends, OpenCL (GPU), CUDA (GPU), TensorRT (GPU), ROCm (GPU), ONNX Runtime (varied hardware), and Eigen (CPU). (On macOS there is also a Metal backend, most easily obtained via homebrew - see above.) On Windows there is also a WinML backend (Windows App SDK, GPU/NPU via EP catalog).
...
The quick summary is:
  ...
  * The WinML backend (Windows only) uses the Windows App SDK / ONNX Runtime EP catalog and is useful for GPU/NPU inference on Windows.
