...
* `neuralnet` - Neural net GPU implementation and interface. Contains OpenCL, CUDA, TensorRT, ROCm, Metal, Eigen, ONNX, and WinML backends along with common interfaces and model data structures.
  ...
  * `{cuda,opencl,eigen,trt,rocm,metal,onnx,winml,dummy}backend.cpp` - Various backends.
  * `onnxmodelbuilder.{cpp,h}` - Builds ONNX graphs from KataGo model weights for ONNX Runtime and WinML backends.
